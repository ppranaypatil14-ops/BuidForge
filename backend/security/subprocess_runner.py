...
from backend.exceptions import ScannerError, ScannerTimeoutError, SupplyGuardError
from backend.security.policy import ACTIVE_SECURITY_POLICY

logger = logging.getLogger("supplyguard.security.subprocess")
...
class SafeSubprocessRunner:
    ...
    def _validate_executable(self, executable: str) -> str:
        ...
        Raises:
            SupplyGuardError: If executable is not allowed or appears to be arbitrary repository code.
        """
        if not executable or not isinstance(executable, str):
            raise SupplyGuardError("Subprocess executable must be a non-empty string.", code="SECURITY_VIOLATION")
    ...
    def run(
        ...
        for idx, arg in enumerate(args):
            if not isinstance(arg, str):
                raise SupplyGuardError(f"Argument at index {idx} must be a string, got {type(arg).__name__}.")
            if "\x00" in arg:
                raise SupplyGuardError(f"Argument at index {idx} contains prohibited null byte.")
